...
> **【图 1 — 放置位置：2011–2012 年每日租赁量总趋势折线图，展示两年间的上升趋势和周期性的季节波动。横轴为日期，纵轴为当日总租赁量，建议叠加一条平滑趋势线。图片来源：app.py Dashboard 页面。】**
...
> **【图 2 — 放置位置：双图。左图为 `temp` 与 `atemp` 全数据散点图（17,379 个点），标注 r=0.987，直观展示极端共线性。右图为四类天气状况的样本量柱状图（11413 / 4544 / 1419 / 3），清晰呈现第四类仅 3 条记录的极端不平衡。图片来源：app.py 数据预处理页面。】**
...
> **【图 3 — 放置位置：并排双直方图，左图为原始 cnt 的分布（偏态+1.277，标注均值189.5和中位数142的竖线），右图为 log(1+cnt) 的分布（偏态−0.818），两图均叠加核密度曲线。这是为对数变换必要性提供最直观论据的图表。图片来源：app.py 数据预处理页面。】**
...
> **【图 4 — 放置位置：平均小时租赁量折线图（横轴 0–23 时），包含两条线——工作日（蓝色）和非工作日（橙色）。这是整个分析中最具视觉冲击力的图表，通勤双峰对比休闲单峰的形态差异在图中一目了然。图片来源：app.py Dashboard 页面。】**
...
> **【图 5 — 放置位置：四个季节的平均小时租赁量柱状图（春=111.1，夏=208.3，秋=236.0，冬=198.9），并在每个柱旁叠加工作日与非工作日的细分色块。秋季而非夏季的需求最高这一反直觉结论，是该图最有讨论价值的发现。图片来源：app.py Dashboard 页面。】**
...
> **【图 6 — 放置位置：XGBoost 的预测值 vs 实际值散点图，点簇应紧密聚集在 y=x 对角线附近。该图比任何指标数字都更直观地传达了模型质量——学生能从图中"看出"预测有多准。图片来源：app.py Deep Learning > XGBoost 页面。】**
...
> **【图 7 — 放置位置：六种模型的 R² 对比柱状图，从左至右按复杂度由低到高排列（线性回归 → 岭回归 → 决策树 → 随机森林 → 梯度提升 → XGBoost）。在 R²=0.90 处画一条虚线标注"优秀阈值"。这是建模部分最重要的一张总结图。图片来源：app.py 总结报告页面。】**
...
*本提案由 Data Mining Group 提交 · 2026 年 3 月*
