...
你是一位数据科学家，现在你有一家公司的自行车租借数据，你需要为公司决策层使用数据赋能

机器学习文件在ml文件夹下，详细阅读，数据挖掘方法.md，数据预处理.md，数据使用hour_wash.csv

首先，你想为他做一个WebUI界面，分为左侧侧边栏（可以收缩），右侧主界面。

在侧边栏中你会做两个选项，分别为dashboard，Deep Learning，summery和测试模型，在Deep Learning下还可以选择几个机器学习的方法。

在dashboard的主界面，你需要对关系数据库进行数据的可视化，并做出批注。

在Deep Learning中每点击一个机器学习的方法，你就跳转到目标界面，可以让公司决策层点击运行此机器学习，你将使用表格与可视化来显示结果。

并且需要输出降维的可视化图，并且标出他的最高点与拐点，如果是随机森林 制作 5-25棵树的准确率，XGBoost 可视化其原理

在summery中，你将总结Deep Learning下面的所有模型，并使用表格与可视化来显示结果，并使用文字做出评论。
...
- 高性能 
- 高可靠
- 需要在docker容器中运行，端口为8300，你需要检查端口是否占用，如果占用请要求用户重新选择端口
- 如果需要python，你可以创建虚拟环境
